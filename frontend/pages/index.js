...
const Home = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      const res = await axios.get("http://localhost:5000/api/items");  // Updated URL
      setItems(res.data);
    };
    fetchItems();
  }, []);
  ...
  const addToCart = async (item) => {
    await axios.post("http://localhost:5000/api/items/cart", item);  // Updated URL
    const res = await axios.get("http://localhost:5000/api/items/cart");  // Updated URL
    setCart(res.data);
  };

  const removeFromCart = async (sku) => {
    await axios.delete(`http://localhost:5000/api/items/cart/${sku}`);  // Updated URL
    const res = await axios.get("http://localhost:5000/api/items/cart");  // Updated URL
    setCart(res.data);
  };

  const updateCartItemQuantity = async (sku, action) => {
    await axios.put(`http://localhost:5000/api/items/cart/${sku}`, { action });  // Updated URL
    const res = await axios.get("http://localhost:5000/api/items/cart");  // Updated URL
    setCart(res.data);
  };
